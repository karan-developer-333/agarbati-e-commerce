      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8">
        <div className="border-y border-gold/20 py-6">
          <div className="flex flex-wrap justify-center gap-x-8 gap-y-6">
            {items.map((item, i) => (
              <div key={i} className="vp-item flex items-start gap-3 min-w-[160px]">
                ...
                {i < items.length - 1 && <div className="hidden lg:block w-px h-10 bg-border-cream self-center ml-2" />}
              </div>
            ))}
          </div>
        </div>
